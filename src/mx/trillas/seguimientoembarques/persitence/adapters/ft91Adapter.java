...
public class ft91Adapter implements JsonSerializer<Ft91> {
	...
	@Override
	public JsonElement serialize(Ft91 ft91, Type arg1, JsonSerializationContext jsc) {
		JsonObject jsonO = new JsonObject();
		try {
			jsonO.addProperty("s", ft91.getSer());
			jsonO.addProperty("f", ft91.getFolio());

			if (ft91.getFoltra() == null || ft91.getFoltra().intValue() == 0) {
				jsonO.addProperty("Traspaso",
						String.format("%-3s", ft91.getId().getSerie()) + String.format("%07d", ft91.getId().getNdoc()));
				jsonO.addProperty("sertra", ft91.getId().getSerie());
				jsonO.addProperty("foltra", ft91.getId().getNdoc());
			} else {
				String traspaso = "";
				traspaso += ft91.getSertra() == null ? "   " : String.format("%-3s", ft91.getSertra());
				traspaso += String.format("%07d", ft91.getFoltra());
				jsonO.addProperty("Traspaso", traspaso);
				jsonO.addProperty("sertra", ft91.getSertra());
				jsonO.addProperty("foltra", ft91.getFoltra());
			}
			jsonO.addProperty("NoTraspasoCasaOrigen",
					String.format("%-3s", ft91.getSertra()) + String.format("%07d", ft91.getFoltra()));
			jsonO.addProperty("FechaTraspaso", ft91.getFecemi().toString());
			String requisicion = "";
			requisicion += ft91.getSerped() == null ? "   " : String.format("%-3s", ft91.getSerped());
			...
			if (ft91.getSer() == null) {
				...
				jsonO.addProperty("Embarque",
						String.format("%-3s", ft91.getSer()) + String.format("%07d", ft91.getFolio()));
